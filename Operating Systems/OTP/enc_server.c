...
int main(int argc, char *argv[]){
  ...
  while(1){
    ...
    char* plaintextBuffer = malloc(plaintextLength + 1);
    recvall(connectionSocket, plaintextBuffer, plaintextLength);
    plaintextBuffer[plaintextLength] = '\0'; // Ensure null termination
    ...
    char* ciphertext = encrypt(plaintextBuffer, keyBuffer, plaintextLength);
    sendall(connectionSocket, ciphertext, plaintextLength + 1); // Send including the null terminator

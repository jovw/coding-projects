...
LIMIT=100000
limit "${bin}"/keygen 70000 > key70000
LIMIT=500
...
((tot+=10))
msg=$(limit "${bin}"/enc_client plaintext1 key20 $encport 3>&1 1>&2 2>&3 3>&-)
printf 'stderr: %s\n' "$msg"
if [ ! -z "$msg" ]
then
	echo "Looks good!"
...
((tot+=20))
limit "${bin}"/enc_client plaintext1 key70000 $encport | tee ciphertext1 | paste 
echo
echo '#-----------------------------------------'
...
((tot+=5))
msg=$(limit "${bin}"/dec_client ciphertext1 key70000 $encport 3>&1 1>&2 2>&3 3>&-)
printf '%s\n' "$msg"
if [ ! -z "$msg" ]
then
	echo "Looks good!"
...
limit "${bin}"/dec_client ciphertext1 key70000 $decport | tee output
...
limit "${bin}"/dec_client ciphertext1 key70000 $decport > plaintext1_a
...
limit 1 "${bin}"/enc_client plaintext1 key70000 $encport > ciphertext1 2>&3 &
limit 2 "${bin}"/enc_client plaintext2 key70000 $encport > ciphertext2 2>&3 &
limit 3 "${bin}"/enc_client plaintext3 key70000 $encport > ciphertext3 2>&3 &
limit 4 "${bin}"/enc_client plaintext4 key70000 $encport > ciphertext4 2>&3 &
limit 5 "${bin}"/enc_client plaintext5 key70000 $encport > ciphertext5 &
wait
__CMDS__
ret=$?
cat empty notempty
...
limit 1 "${bin}"/dec_client ciphertext1 key70000 $decport > plaintext1_a &
limit 2 "${bin}"/dec_client ciphertext2 key70000 $decport > plaintext2_a &
limit 3 "${bin}"/dec_client ciphertext3 key70000 $decport > plaintext3_a &
limit 4 "${bin}"/dec_client ciphertext4 key70000 $decport > plaintext4_a &
wait
__CMDS__
ret=$?
if [ $ret -eq 124 ]

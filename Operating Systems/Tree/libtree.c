...
/* TODO: START HERE */
static int
/* fileinfo struct has the stats of the current file */
tree_print_recurse(struct fileinfo finfo)
{
  ...
  /* Print out only directories and no files - if fileinfo struct is not a directory, return here */
  if (opts.dirsonly && !S_ISDIR(finfo.st.st_mode)) {
    return 0;
  }

  /* TODO: print indentation */
  /* product of the current depth and unsigned indent in libtree.h */
  for (int i = 0; i < opts.indent * depth; ++i) 
    putchar(' ');

  /* TODO: print the path info */
  /* use function - print_path_info -> in this function, how do you print each 
   * ex the permissions, user, group, size - use the hints */
  if (print_path_info(finfo) == -1) 
    goto exit;

  /* if this is a file, we are done, return here */
  ...
  if (read_file_list(dirp, &file_list, &file_count) == -1) {
    ...
    goto exit; /* only exit if an error occur*/
  }
...
print_path_info(struct fileinfo finfo)
{
  char sep = '[';
  if (opts.perms) {
    /* take a look ath the mode_string function */
    char *modeString = mode_string(finfo.st.st_mode);
    if (modeString == NULL)
      return -1;

    if (printf("%c%s", sep, modeString) < 0) 
      goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.user) {
...
filecmp(void const *_lhs, void const *_rhs)
{
  ...
  switch (opts.sort) {
    ...
    case ALPHA:
      ...
      break; /* TODO - refer to the RALPHA  */
    case RALPHA:
...
read_file_list(DIR *dirp, struct fileinfo **file_list, size_t *file_count)
{
  for (;;) {
    ...
    /* TODO: Skip hidden files - only when the -a is not used. If -a option is used pront out the hidden files */
    if (!opts.all && de->d_name[0] == '.' && de->d_name[1] != '\0')
        continue;
